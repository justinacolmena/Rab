...
This is very easy to do in \textsf{R}. The coefficient labeled ``\mbox{\texttt{(Intercept)}}'' is known as $\alpha$ (``alpha'') --- in this case a \textit{daily} $\alpha$ which would have to be multiplied by $365$ to yield an annual ``stock market $\alpha$'' --- except that this is not always statistically significant in a linear regression of daily stock market returns.

The coefficient labeled ``\mbox{\texttt{x}}'' is the $\beta$ (``beta'') of the stock whose returns we are modeling against those of the S\&P500.

<<>>=
model1 <- lm(formula = y ~ x, data = j)
summary(model1)
@
...
@
\caption[Plot of linear model]{Scatter plot of linear model with line of best fit}
\end{figure}
